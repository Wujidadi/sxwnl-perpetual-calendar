...
| `ditu2` (字串)      | 世界各國國界線資料（與 `ditu1` 海岸線同編碼，畫法不同色 `#D0D0FF`） | `WORLD_BORDERS`      |
...
| `Ck` / `Ck0`        | 原 `sm6.htm`（演算法附錄「離散序列直線擬合」章節）輸入欄位：斜率 k／起始 k0 | 新版未引入該章節      |
| `Cn`                | 原 `sm6.htm` 輸入欄位：點數 N                                               | 新版未引入該章節      |
| `pan_1`             | 面板                                                                        | `panel-1`             |
