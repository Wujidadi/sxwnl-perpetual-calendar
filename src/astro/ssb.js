// 太陽系質心（SSB）：地球的速度與座標多項式表，供恆星章動與光行差計算。

import { RAD_TO_ARCSEC } from './constants.js';

export const EARTH_SSB_VELOCITY_TABLE = new Array( // 地球的 SSB 速度計算表
  ...
  1,  1,   0, 0,  0, 0,       -2,     0,   -13,     156,    32,   -358, // 本行開始為泊松項
  2,  1,   0, 0,  0, 0,      141,  -107,   -95,    -130,   -48,    -55,
  3,  1,   0, 0,  0, 0,       -5,    -4,    -4,       5,     0,      0
);


// 地球 SSB 速度，t 為儒略世紀數；單位 AU/世紀，平均誤差 4×10⁻⁸ AU/日。
export function earthSSBVelocity(t) {
  const J = [
  ...
  for (let i = 0; i < 39; i++) {
    ...
    const S = Math.sin(c), C = Math.cos(c);
...
export const EARTH_SSB_POSITION_TABLE = new Array(
...
// 地心 SSB 座標，t 為儒略世紀數；4 位有效數字。
export function earthSSBPosition(t) {
  t /= 10;
  const p = EARTH_SSB_POSITION_TABLE;
  let x = 0, y = 0, z = 0;
  for (let i = 0; i < p.length; i += 6) {
    x += p[i + 0] * Math.cos(p[i + 1] + p[i + 2] * t);
    y += p[i + 3] * Math.cos(p[i + 4] + p[i + 5] * t);
  }
