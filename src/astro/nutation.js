// 章動計算（IAU2000B 高精度與中精度）。

import { RAD_TO_ARCSEC } from './constants.js';
import { normalizeAngle } from './math-utils.js';

// IAU2000B 章動序列：l, l', F, D, Om, A, A', A'', B, B', B''
export const NUTATION_IAU2000B_SERIES = [
...
// 高精度章動，t 為 J2000 起算儒略世紀數；
// minPeriodDays（原 zq）非 0 時只計算週期大於該值（天）的項。
// 回傳 [dL, dE]：黃經章動、交角章動（弧度）。
export function nutation(t, minPeriodDays) {
  const t2 = t * t, t3 = t2 * t, t4 = t3 * t;
  const l  =  485868.249036 + 1717915923.2178 * t + 31.8792  * t2 + 0.051635 * t3 - 0.00024470 * t4;
  ...
  const D  = 1072260.70369  + 1602961601.2090 * t -  6.3706  * t2 + 0.006593 * t3 - 0.00003169 * t4;
  ...
  for (let i = 0; i < 77 * 11; i += 11) {
    const c = (B[i] * l + B[i + 1] * l1 + B[i + 2] * F + B[i + 3] * D + B[i + 4] * Om) / RAD_TO_ARCSEC;
    if (minPeriodDays) {
      const q = 36526 * 2 * Math.PI * RAD_TO_ARCSEC / (1717915923.2178 * B[i] + 129596581.0481 * B[i + 1] + 1739527262.8478 * B[i + 2] + 1602961601.2090 * B[i + 3] + 6962890.5431 * B[i + 4]);
...
// 把章動量套用到赤道座標。z 為赤道座標、E 為黃赤交角、dL 黃經章動、dE 交角章動。
export function applyEquatorialNutation(z, E, dL, dE) {
  const r = [z[0], z[1], z[2]];
  r[0] += (Math.cos(E) + Math.sin(E) * Math.sin(z[0]) * Math.tan(z[1])) * dL - Math.cos(z[0]) * Math.tan(z[1]) * dE;
  r[1] += Math.sin(E) * Math.cos(z[0]) * dL + Math.sin(z[0]) * dE;
  r[0] = normalizeAngle(r[0]);
  return r;
}

// 中精度章動表
export const NUTATION_MEDIUM_TABLE = [
...
// 中精度章動，回傳 [dL, dE]。
export function nutationMedium(t) {
  const t2 = t * t;
...
// 中精度黃經章動。
export function nutationLongitudeMedium(t) {
  const t2 = t * t;
  const B = NUTATION_MEDIUM_TABLE;
  let dL = 0;
  for (let i = 0; i < B.length; i += 5) {
