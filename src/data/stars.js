...
// 字串內容含中英混雜，Phase 4 i18n 時拆分中文／英文欄位。

import { RAD_TO_ARCSEC } from '../astro/constants.js';
import { parseAngleToRadian } from '../astro/angle-format.js';

export const CONSTELLATIONS_88 = // 88星座表。結構：漢語+縮寫, 面積(平方度), 赤經 (時分), 赤緯(度分), 象限角 族 星座英文名
...
export const STAR_CATALOG = new Array( // 恆星庫
  // RA(時分秒)   DEC(度分秒)   自行1  自行2  視差  星等  星名  星座#
...
// 拆解 88 星座資料（init 階段呼叫一次即可）。
const CONSTELLATIONS_88_PARSED = CONSTELLATIONS_88.split(',');

// 星庫檢索：依關鍵字 key 從各子庫中蒐集匹配記錄；亦會附上 88 星座的中心位置條目。
export function searchStarCatalog(key) {
  let r = '';
  for (let i = 0; i < STAR_CATALOG.length; i++) {
    ...
    let n2, n3;
    while (true) {
      n1 = s.indexOf(key, n1 + 1); if (n1 === -1) break;
      n2 = s.indexOf('#', n1);     if (n2 === -1) n2 = s.length;
      n3 = s.lastIndexOf('#', n1); if (n3 === -1) n3 = n0;
      r += s.substr(n3, n2 - n3);
    }
  }
  const xz = CONSTELLATIONS_88_PARSED;
  for (let i = 0; i < xz.length; i += 5) {
    if (xz[i].substr(3, 3) === key) {
      let a = xz[i + 2], b = xz[i + 3];
      a = a.substr(0, 5) + ' ' + (a.substr(6, 2) * 0.6).toFixed(1);
      b = b.substr(0, 6) + ' ' + (b.substr(7, 2) * 0.6).toFixed(1);
      r = '#*' + a + ',' + b + ',0,0,0,0.0,中心' + xz[i + 1] + '方,' + xz[i + 4] + r;
      break;
    }
  }
  return r;
...
// 從檢索結果字串提取並格式化恆星資料（度分秒／角分秒→弧度）。
// all=1 表示全部取出（含未以 * 標記者）。
export function parseStarCatalog(s, all) {
  s = s.replace(/\r/g, '');
  s = s.replace(/\n/g, '#');
  const sn = s.indexOf('#') + 1;
  ...
  for (let i = 0; i < parts.length; i += 8) {
    if (!parts[i] || parts[i].length < 5) continue;
    if (parts[i].substr(0, 1) !== '*' && !all) continue;
    parts[i] = parts[i].substr(1, parts[i].length - 1);
    r[k++] = parseAngleToRadian(parts[i + 0], 1);
    r[k++] = parseAngleToRadian(parts[i + 1], 0);
    r[k++] = parts[i + 2] / RAD_TO_ARCSEC * 15;
    r[k++] = parts[i + 3] / RAD_TO_ARCSEC;
    r[k++] = parts[i + 4] / RAD_TO_ARCSEC;
    r[k++] = parts[i + 5];
    r[k++] = parts[i + 6];
    r[k++] = parts[i + 7];
  }
