...
import { formatTimeOfDay } from '../astro/julian-day.js';
import {
...
export function getReignTitle(y) {
  let s = '';
...
export function getLunarDayName(u, r) {
  ...
  if (u.solarTermLabel) {
    ...
    else                                r.holidayB += u.solarTermLabel + ' ';
  }
...
export function computeBazi(jd, J, ob) {
  ...
  ob.baziTrueSolarTime = formatTimeOfDay(jd);
...
export function preciseSolarTermFromLongitude(W) {
  const t = sunApparentLongToTime(W) * 36525;
  return t - deltaT(t) + 8 / 24;
}

// 精朔（給定月日視黃經差 W，回傳合朔 JD，已加上北京時 +8h）。
export function preciseNewMoonFromLongitude(W) {
  const t = moonSunDiffToTime(W) * 36525;
  return t - deltaT(t) + 8 / 24;
}

// 精氣（給定 JD 求最近節氣）。
export function preciseSolarTermFromJD(jd) {
  const d = Math.PI / 12;
...
// 精朔（給定 JD 求最近合朔）。
export function preciseNewMoonFromJD(jd) {
